...
export const InventoryView: React.FC<InventoryViewProps> = ({
  ...
  const handleLoadRealCatalog = async () => {
    if (window.confirm('Import your full Real_product.csv catalog (120+ items from INK, Field, etc.) into local storage?')) {
      try {
        const response = await fetch('/Booth_POS/Real_product.csv');
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950 overflow-hidden select-none text-slate-100">
      ...
      <div className="bg-slate-900 border-b border-slate-800 p-5 shrink-0 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-white tracking-tight">Product Catalog & Setup</h1>
          <p className="text-xs text-slate-400 mt-0.5">
            {products.length} products total • Local photos & offline database
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-750 text-slate-200 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
          >
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-xl border border-emerald-500/30 bg-emerald-950/40 hover:bg-emerald-900/50 text-emerald-300 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
          ...
          <label className="px-3.5 py-2.5 rounded-xl border border-indigo-500/30 bg-indigo-950/40 hover:bg-indigo-900/50 text-indigo-300 font-bold text-xs flex items-center gap-1.5 transition-colors cursor-pointer">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 active:scale-98 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-indigo-600/30 transition-all cursor-pointer"
          >
      ...
      {/* Success banner */}
      {importStatus && (
        <div className="bg-emerald-950/60 border-b border-emerald-800 text-emerald-300 px-5 py-2 text-xs font-bold flex items-center gap-2">
      ...
      <div className="bg-slate-900 border-b border-slate-800 px-5 py-3 flex items-center gap-3 shrink-0">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            ...
            className="w-full bg-slate-800 pl-9 pr-3 py-2 rounded-xl text-xs border border-slate-700 text-white placeholder:text-slate-500 focus:bg-slate-750 focus:border-indigo-500 outline-none"
          />
        </div>
        ...
        <select
          ...
          className="bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs font-semibold text-slate-200 outline-none cursor-pointer"
        >
              ...
              {filteredProducts.length === 0 ? (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-slate-500">
                    No products found. Tap "Add Product" or "Sample Data" to start!
                  </td>
                </tr>
              ) : (
                    ...
                    <tr key={prod.id} className="hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          <div className="w-11 h-11 rounded-xl bg-slate-800 border border-slate-700 flex items-center justify-center shrink-0 overflow-hidden text-lg">
                            {prod.image ? (
                      ...
                      <td className="py-3 px-3">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="w-6 h-6 rounded-md bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold flex items-center justify-center text-xs cursor-pointer"
                          >
                            -
                          </button>
                          <span className={`min-w-[32px] text-center font-bold ${prod.stock <= 5 ? 'text-rose-400' : 'text-slate-100'}`}>
                            {prod.stock}
                          </span>
                          <button
                            onClick={() => prod.id && handleQuickStockChange(prod.id, prod.stock, 1)}
