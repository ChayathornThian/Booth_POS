...
export const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return (
    <button
      ...
      className={`group relative flex flex-col justify-between text-left bg-white rounded-2xl border-2 transition-all duration-150 overflow-hidden select-none active:scale-97 p-3 shadow-xs hover:shadow-md ${
        isOutOfStock
    ...
    >
      ...
      {cartQuantity > 0 && (
        <div className="absolute top-2 right-2 z-10 w-7 h-7 rounded-full bg-indigo-600 text-white font-black text-xs flex items-center justify-center shadow-md animate-in fade-in zoom-in duration-150">
          {cartQuantity}
        </div>
      )}

      {/* Top Section: Visual Media (Local Image or Large Emoji) */}
      <div className="w-full h-24 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden mb-2 relative">
        {product.image ? (
          <img
            src={product.image}
            alt={product.name}
        ...
        <div className="absolute bottom-1.5 left-1.5">
          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md border ${artistStyle.bg} ${artistStyle.text} ${artistStyle.border} shadow-2xs`}>
            {product.artist}
          </span>
        </div>
      </div>
      ...
      <div className="mt-2 pt-2 border-t border-slate-100 flex items-end justify-between">
        ...
        {product.isBundle && product.bundleQty && product.bundlePrice && (
          <div className="flex items-center gap-1 text-[10px] font-bold bg-amber-100 text-amber-800 border border-amber-300 px-1.5 py-0.5 rounded-md">
