...
export const CloudSyncModal: React.FC<CloudSyncModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-150 select-none font-sans">
      <div className="bg-slate-900 rounded-3xl w-full max-w-lg shadow-2xl border border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-xl font-black text-white">Cloud Database & Live Monitor</h2>
                <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
                  isCloudConnected ? 'bg-emerald-950/80 text-emerald-400 border-emerald-800/60' : 'bg-slate-800 text-slate-400 border-slate-700'
                }`}>
                  {isCloudConnected ? 'Connected' : 'Offline / Standby'}
                </span>
              </div>
              <p className="text-xs text-slate-400">Real-time sync to Firebase for separate remote dashboards</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Body */}
        <div className="p-6 overflow-y-auto space-y-4.5 flex-1 text-xs text-slate-300">
          {uploadMessage && (
            <div className="p-3.5 rounded-2xl bg-emerald-950/50 border border-emerald-800/60 text-emerald-300 font-bold flex items-center gap-2">
          ...
          <div className="p-4 bg-slate-950/70 rounded-2xl border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <span className="font-extrabold text-white text-sm block">Enable Cloud Real-Time Sync</span>
              ...
              <input
                ...
                className="w-5 h-5 text-indigo-500 rounded-sm focus:ring-indigo-500 cursor-pointer accent-indigo-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                placeholder="boothsuay"
                className="w-full bg-slate-900 border border-slate-700 rounded-xl px-3.5 py-2 text-xs font-mono font-bold text-white outline-none focus:border-indigo-500"
              />
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-semibold text-indigo-400">Auto-fills all fields below</span>
            </div>
            <textarea
              rows={3}
              value={configText}
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-[11px] font-mono text-slate-300 outline-none focus:bg-slate-900 focus:border-indigo-500 placeholder:text-slate-600"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              <label className="text-[11px] font-bold text-slate-400 block mb-1">API Key *</label>
              <input
                type="text"
                value={apiKey}
                onChange={e => setApiKey(e.target.value)}
                placeholder="AIzaSy..."
                className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs font-mono text-white outline-none focus:border-indigo-500 placeholder:text-slate-600"
              />
            </div>
            <div>
              <label className="text-[11px] font-bold text-slate-400 block mb-1">Project ID *</label>
              <input
                type="text"
                value={projectId}
            ...
            <div>
              <label className="text-[11px] font-bold text-slate-400 block mb-1">App ID *</label>
              <input
                type="text"
                value={appId}
            ...
            <div>
              <label className="text-[11px] font-bold text-slate-400 block mb-1">Auth Domain</label>
              <input
                type="text"
                value={authDomain}
          ...
          <div className="pt-2 flex flex-col gap-2">
            <button
              onClick={handleSave}
              className="w-full py-3.5 rounded-2xl bg-indigo-600 hover:bg-indigo-500 active:scale-98 text-white font-extrabold text-xs flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/30 transition-all cursor-pointer"
            >
            ...
            {isCloudConnected && (
              <button
                ...
                className="w-full py-3 rounded-2xl border border-indigo-500/40 bg-indigo-950/40 hover:bg-indigo-900/50 text-indigo-300 font-bold text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
              >
